...
def private_key_to_pem(private_key_int):
    # Convert the private key integer to a cryptography RSA key object
...
################ file PEM #################
def makeKeyFilesPem(name, sizeofkey):
# Creates two files 'x_pubkey.txt' and 'x_privkey.txt' (where x
    # is the value in name) with the n,e and d,e integers written in
    # them, delimited by a comma.
    ...
    if os.path.exists('%s_pubkey.pem' % (name)) or os.path.exists('%s_privkey.pem' % (name)):
        sys.exit('WARNING: The file %s_pubkey.pem or %s_privkey.pem already exists! Use a different name or delete these files and re-run this program.' % (name, name))

    publicKey, privateKey = generateKey(sizeofkey)

    print()
    print('The public key is a %s and a %s digit number.' % (len(str(publicKey[0])), len(str(publicKey[1]))))
    print('Writing public key to file %s_pubkey.pem...' % (name))
    fo = open('d:\SSI\s2\crypto\projet\%s_pubkey.pem' % (name), 'w')
    fo.write('-----BEGIN PUBLIC KEY----- \n')
    nombre_utf8 = str(publicKey[1]).encode('ascii')   
    base64_bytes = base64.b64encode(nombre_utf8)
    base64_text = base64_bytes.decode('ascii') 
    fo.write('%s \n' % (base64_text))
    fo.write('-----END PUBLIC KEY-----')
    fo.close()

    print()
    print('The private key is a %s and a %s digit number.' % (len(str(publicKey[0])), len(str(publicKey[1]))))
    print('Writing private key to file %s_privkey.pem...' % (name))
    fo = open('d:\SSI\s2\crypto\projet\%s_privkey.pem' % (name), 'w')
    fo.write('-----BEGIN PRIVATE KEY----- \n')
    nombre_utf8 = str(privateKey[1]).encode('ascii')   
    base64_bytes = base64.b64encode(nombre_utf8)
    base64_text = base64_bytes.decode('ascii')
    fo.write('%s \n' % (base64_text))
    fo.write('-----END PRIVATE KEY-----')
    fo.close()
    
    
#makeKeyFilesPem('ahmed',2048)
